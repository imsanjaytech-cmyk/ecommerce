        ...
        function renderTable(products, meta) {
            ...
            productsBody.innerHTML = products.map(p => `
                <tr data-id="${p.id}">
                    ...
                    <td class="col-product">
                        <div style="display:flex;align-items:center;gap:11px;">
                            <div style="width:42px;height:42px;border-radius:10px;overflow:hidden;border:1.5px solid var(--border-col);flex-shrink:0;background:var(--pink-soft);">
                                ${p.thumbnail_url
                                    ? `<img src="${p.thumbnail_url}" alt="${escHtml(p.name)}" style="width:100%;height:100%;object-fit:cover;">`
                                    : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;"><i class="bi bi-box-seam" style="color:var(--secondary);font-size:16px;"></i></div>`
                                }
                            </div>
                            <div>
                                <div class="prod-name">${escHtml(p.name)}</div>
                                <div class="prod-meta">${p.created_at}</div>
                            </div>
                        </div>
                    </td>
                    <td class="col-sku" style="font-size:12px;color:var(--text-muted);font-family:monospace;">${escHtml(p.sku)}</td>
                    ...
                    <td class="col-feat">
                        <button class="feat-btn ${p.is_featured ? 'on' : ''}" data-id="${p.id}" title="Toggle featured">
                            <i class="bi ${p.is_featured ? 'bi-star-fill' : 'bi-star'}"></i>
                        </button>
                    </td>
                    <td style="text-align:right;padding-right:12px;">
                        <div class="act-row" style="justify-content:flex-end;">
                            <div class="act-btn edit-btn" data-id="${p.id}" title="Edit"><i class="bi bi-pencil"></i></div>
                            <div class="act-btn del delete-btn" data-id="${p.id}" data-name="${escHtml(p.name)}" title="Delete"><i class="bi bi-trash"></i></div>
                        </div>
                    </td>
                </tr>
            `).join('');
